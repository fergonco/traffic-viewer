...
speeds <-read.csv("tocern.csv", colClasses = c(holidayfr="logical", holidaych="logical",schoolfr="logical",schoolch="logical"))
...
facetedScatterplots <- ggplot(data = speeds, aes(x=minutes, y=speed)) + geom_point(shape=1) + ggtitle("results") + facet_grid(weekday~morningrush, labeller = label_both)
